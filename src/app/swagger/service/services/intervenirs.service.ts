...
import { IntervenirDto } from '../models/intervenir-dto';
...
export class IntervenirsService extends BaseService {
  ...
  context?: HttpContext
...
): Observable<StrictHttpResponse<Array<IntervenirDto>>> {
    ...
    return this.http.request(rb.build({
      ...
      map((r: HttpResponse<any>) => {
        return r as StrictHttpResponse<Array<IntervenirDto>>;
      })
    );
  }
...
): Observable<Array<IntervenirDto>> {

    return this.getAll2$Response(params,context).pipe(
      map((r: StrictHttpResponse<Array<IntervenirDto>>) => r.body as Array<IntervenirDto>)
    );
  }
  ...
  context?: HttpContext
...
): Observable<StrictHttpResponse<IntervenirDto>> {
    ...
    return this.http.request(rb.build({
      ...
      filter((r: any) => r instanceof HttpResponse),
      map((r: HttpResponse<any>) => {
        return r as StrictHttpResponse<IntervenirDto>;
      })
    );
  }
...
): Observable<IntervenirDto> {

    return this.insert5$Response(params,context).pipe(
      map((r: StrictHttpResponse<IntervenirDto>) => r.body as IntervenirDto)
    );
  }
